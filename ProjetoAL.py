...
# Matriz A (Coeficientes de consumo)
A = [[5.0, 1.0, 1.0],
     [3.0, 4.0, 1.0],
     [2.0, 1.0, 4.0]]
...
def copiar_matriz(matriz): # faz uma copia da matriz 
    return [linha[:] for linha in matriz]

def multiplicar_vetor(v1, v2): # realiza o produto interno dos vetores
    return sum(a * b for a, b in zip(v1, v2))

def norma_infinito(vetor): # obtem a norma
    return max(abs(x) for x in vetor)

def subtrair_vetores(v1, v2): # realiza a subtração do vetor
...
def eliminacao_gauss(A, b):
    ...
    for i in range(n): # Eliminação Progressiva (transformar matriz em triangular superior)
        ...
        for j in range(i + 1, n): 
            if Ab[i][i] == 0: continue # Se o pivo for = 0, ele pula 
...
# CONCLUSÃO
print("\n" + "="*70)
print("CONCLUSÃO FINAL — META IDEAL DE EVENTOS PARA 5 ANOS")
print("="*70)
